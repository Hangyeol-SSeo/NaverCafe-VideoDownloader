    ...
    chrome.storage.local.get(String(tabId), function (result) {
      ...
      if (result[String(tabId)]) {
        chrome.storage.local.remove(String(tabId), function () {
          console.log(
            `videoSources data removed from local storage for tab ${tabId} due to page update`
          );
        });
      }
    });
...
chrome.tabs.onRemoved.addListener(function (tabId, removeInfo) {
  chrome.storage.local.get(String(tabId), function (result) {
    ...
    if (result[String(tabId)]) {
      chrome.storage.local.remove(String(tabId), function () {
        console.log(
          `videoSources data removed from local storage for tab ${tabId} due to tab removal`
        );
      });
    }
  });
